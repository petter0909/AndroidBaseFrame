...
public class WheelView extends View {
    ...
    protected void notifyClickListenersAboutClick(int item) {
        if (clickingListeners != null && clickingListeners.size() > 0) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isClick) {
                    float distance = (event.getX() - touchDownX) * (event.getX() - touchDownX) + (event.getY() - touchDownY) * (event.getY() - touchDownY);
                    if (distance > 100) {
                        isClick = false;
                    }
                }
    ...
    private void doScroll(int delta) {
        ...
        int itemCount = viewAdapter == null ? 0 : viewAdapter.getItemsCount();
        ...
        if (!isCyclic && ((currentItem == 0 && scrollingOffset > 0) || (currentItem == itemCount - 1 && scrollingOffset < 0))) {
            scrollingOffset = 0;
        }
    ...
    private boolean rebuildItems() {
        ...
        if (range != null) {
            if (itemsLayout != null && recycle != null) {
            ...
            if (!updated) {
                updated = firstItem != range.getFirst()
                        || (itemsLayout != null && itemsLayout.getChildCount() != range.getCount());
            }
    ...
    private void highLightSelectItem() {
        ...
        if (itemsLayout != null && (currentItem - firstItem) >= 0) {
            View v = null;
            if (range != null && range.contains(lastSelectItem)) {
