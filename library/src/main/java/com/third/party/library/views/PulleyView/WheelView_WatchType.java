 ...
 * 3d滚轮控件
 */
public class WheelView_WatchType extends View {
    ...
    OnItemSelectedListener_WatchType onItemSelectedListenerWatchType;
    ...
    public WheelView_WatchType(Context context) {
        this(context, null);
    }

    public WheelView_WatchType(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    //    customTextSize = getResources().getBoolean(R.bool.pickerview_customTextSize);
        if(attrs != null) {
            ...
            textSize = a.getDimensionPixelOffset(R.styleable.wheelview_textSize1,textSize);
        }
    ...
    private void initLoopView(Context context) {
        ...
        handler = new MessageHandler_WatchType(this);
        gestureDetector = new GestureDetector(context, new LoopViewGestureListener_WatchType(this));
    ...
    private void remeasure() {
        ...
        if (initPosition == -1) {
            if (isLoop) {
                initPosition = (adapter.getCount() + 1) / 2;
            } else {
                initPosition = 0;
            }
        }
    ...
    void smoothScroll(ACTION action) {
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new SmoothScrollTimerTask_WatchType(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    protected final void scrollBy(float velocityY) {
        cancelFuture();

        mFuture = mExecutor.scheduleWithFixedDelay(new InertiaTimerTask_WatchType(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
    ...
    public final void setOnItemSelectedListenerWatchType(OnItemSelectedListener_WatchType OnItemSelectedListener_WatchType) {
        this.onItemSelectedListenerWatchType = OnItemSelectedListener_WatchType;
    }

    public final void setAdapter(TypeAdapter adapter) {
    ...
    protected final void onItemSelected() {
        if (onItemSelectedListenerWatchType != null) {
            postDelayed(new OnItemSelectedRunnable_WatchType(this), 200L);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            preCurrentIndex = initPosition + change % adapter.getCount();
        ...
        if (!isLoop) {//不循环的情况
            ...
            if (preCurrentIndex > adapter.getCount() - 1) {
                preCurrentIndex = adapter.getCount() - 1;
            }
        } else {//循环
            if (preCurrentIndex < 0) {//举个例子：如果总数是5，preCurrentIndex ＝ －1，那么preCurrentIndex按循环来说，其实是0的上面，也就是4的位置
                preCurrentIndex = adapter.getCount() + preCurrentIndex;
            }
            if (preCurrentIndex > adapter.getCount() - 1) {//同理上面,自己脑补一下
                preCurrentIndex = preCurrentIndex - adapter.getCount();
            }
        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                    String preSelectedItem = adapter.indexOf(visibles[counter]);
                    if(!"".equals(preSelectedItem)){
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!isLoop) {
                    ...
                    float bottom = (adapter.getCount() - 1 - initPosition) * itemHeight;
