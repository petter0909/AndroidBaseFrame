...
final class InertiaTimerTask_WatchType extends TimerTask {

    float a;
    final float velocityY;
    final WheelView_WatchType loopView;

    InertiaTimerTask_WatchType(WheelView_WatchType loopview, float velocityY) {
        super();
        loopView = loopview;
    ...
    public final void run() {
        ...
        if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
            ...
            if (loopView.handler != null) {
                loopView.handler.sendEmptyMessage(MessageHandler_WatchType.WHAT_SMOOTH_SCROLL);
            }
            return;
        }
