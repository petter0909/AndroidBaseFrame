...
public class BottomNavigationBar extends FrameLayout {
    ...
    private boolean FixedisStartAnimator = true;
...
//    }
    ...
    /**
     * @param FixedisStartAnimator 是否显示 字体缩放动画 icon位移动画
     * @return
     */
    public BottomNavigationBar setFixedAnimatorIsStart(boolean FixedisStartAnimator) {
        this.FixedisStartAnimator = FixedisStartAnimator;
        return this;
    }
        ...
        if (mSelectedPosition != newPosition) {
            if (mBackgroundStyle == BACKGROUND_STYLE_STATIC) {
                if (mSelectedPosition != -1)
                    mBottomNavigationTabs.get(mSelectedPosition).unSelect(true, mAnimationDuration,FixedisStartAnimator);
                mBottomNavigationTabs.get(newPosition).select(true, mAnimationDuration,FixedisStartAnimator);
            } else if (mBackgroundStyle == BACKGROUND_STYLE_RIPPLE) {
                if (mSelectedPosition != -1)
                    mBottomNavigationTabs.get(mSelectedPosition).unSelect(false, mAnimationDuration,FixedisStartAnimator);
                mBottomNavigationTabs.get(newPosition).select(false, mAnimationDuration,FixedisStartAnimator);
