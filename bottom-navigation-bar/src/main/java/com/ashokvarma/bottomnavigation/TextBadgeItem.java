...
public class TextBadgeItem extends BadgeItem<TextBadgeItem> {
     ...
     * @return
     */
    public TextBadgeItem setShape(@ColorRes int shape) {
        this.mBadgeItemShape = shape;
        refreshDrawable();
        return this;
    }
    ...
    /**
     * @param size text to be set in badge (this shouldn't be empty text)
     * @return this, to allow builder pattern
     */
    public TextBadgeItem setTextSize(@Nullable int size) {
        if (isWeakReferenceValid()) {
            TextView textView = getTextView().get();
     ...
     * @param mCornerRadius
     */
    public TextBadgeItem setCornerRadius(int mCornerRadius) {
    ...
    public int getmBadgeItemShape() {
        return mBadgeItemShape == 0 ? GradientDrawable.RECTANGLE : mBadgeItemShape;
    }
    ...
    private int getCornerRadius() {
        if (mCornerRadius==0){
            return getTextView().get().getContext().getResources().getDimensionPixelSize(R.dimen.badge_corner_radius);
        }
    ...
    private GradientDrawable getBadgeDrawable(Context context) {
        ...
        shape.setShape(getmBadgeItemShape());
